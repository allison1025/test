...
```
Study Limitations:
In addition to test scores and GPA, an applicant's accomplishments in extracurriculars, strength of statement of purpose response and reccommendation letters are also important criteria in deciding one's admission decision. However, these are factors that a machine learning model cannot consider. For instance, it would be difficult to develop a model that could read in an applicant's essay and decide whether it is well-written, since that is a subjective opinion.
